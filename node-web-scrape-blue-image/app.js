...
const fs = require('fs');
const url = 'https://unsplash.com/photos/V4uS8SqCuNM';
const jsonPath = "./json/"; 
    ...
    for(let i = 0; i < aTags.length; i++){
        ...
        if('href' in attrs){
            ...
            if(hrefValue.indexOf('index.php') == -1 && hrefValue[0] != '#' ){
                ...
                if(hrefValue.indexOf('/photos/') != -1 && hrefValue.indexOf('.com') == -1){
                    hrefValue = 'https://unsplash.com'+hrefValue;
                }
...
function getAllImages(soupTag){
    ...
    for(let i = 0; i < imgs.length; i++){
        ...
        if('src' in attrs){
            let src = attrs.src;
            if(src.indexOf("photos/Special:") == -1){ //these are not images
...
function createSoup(document){
    ...
    let cp0N4 = soup.find('div', { class: 'cp0N4'});

    //find get an element by id
    let btXSB = soup.find('div', { class: 'btXSB' });
    // let classExample = soup.findAll('div', { class: 'className' });//returns array of tags
 
    let images = getAllImages(btXSB);

    data.content = {
        "externalLinks": getAllExternalLinks(btXSB),
        "text": getParagraphText(cp0N4),
